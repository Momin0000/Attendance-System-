...
from app.models.models import Student, AttendanceRecord, Campus, Batch, Course
...
async def dashboard(
    ...
    today = date.today()
    today_records = db.query(AttendanceRecord).filter(AttendanceRecord.attendance_date == today).all()
    ...
    for i in range(6, -1, -1):
        ...
        recs = db.query(AttendanceRecord).filter(AttendanceRecord.attendance_date == day).all()
    ...
    for c in campuses:
        ...
        c_recs = db.query(AttendanceRecord).filter(AttendanceRecord.campus_id == c.id).all()
...
async def batch_analytics(
    ...
    records = db.query(AttendanceRecord).filter(AttendanceRecord.batch_id == batch_id).all()
...
async def low_attendance(
    ...
    result = []
    for s in students:
        recs = db.query(AttendanceRecord).filter(AttendanceRecord.student_db_id == s.id).all()
        if not recs:
            continue
        present = sum(1 for r in recs if r.status == "present")
