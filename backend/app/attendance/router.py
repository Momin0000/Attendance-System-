...
from app.models.models import Student, Attendance, QRCode
...
def _duplicate_check(db: Session, student_db_id: int, attendance_date: date):
    existing = db.query(Attendance).filter(
        Attendance.student_db_id == student_db_id,
...
async def daily_attendance(
    ...
    q = db.query(Attendance).filter(Attendance.attendance_date == target)
    if batch_id:
        q = q.filter(Attendance.batch_id == batch_id)
    if campus_id:
        q = q.filter(Attendance.campus_id == campus_id)
    return q.order_by(Attendance.check_in_time.desc()).all()
...
async def student_history(
    ...
    q = db.query(Attendance).filter(Attendance.student_db_id == student_db_id)
    if from_date:
